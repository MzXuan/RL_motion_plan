...
class PlaneScene(Scene):
	...
	def episode_restart(self, bullet_client):
		...
		if self.stadiumLoaded == 0:
			...
			self.ground_plane_mjcf = self._p.loadURDF(os.path.join(assets.getDataPath(), "scenes_data", "plane/plane.urdf"), [0, 0, -1.5])
...
class StadiumScene(Scene):
	...
	def episode_restart(self, bullet_client):
		...
		if self.stadiumLoaded == 0:
			...
			filename = os.path.join(assets.getDataPath(), "scenes_data", "stadium", "plane_stadium.sdf")

			self.ground_plane_mjcf=self._p.loadSDF(filename)

			filename = os.path.join(assets.getDataPath(),"scenes_data", "stadium", "stadium.sdf")
			self.ground_plane_mjcf = self._p.loadSDF(filename)
			#
